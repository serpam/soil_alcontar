...
raw_soil <- readxl::read_excel(
  ...
  mutate(
    ...
    pre_post_quema = case_when(
      pre_post_quema == "Prequema" ~ "0 Pre",
      pre_post_quema == "Postquema" ~ "1 Post"
    )
  ) 
...
medias <- soil %>% dplyr::select(
  pre_post_quema, zona, humedad:p_h_k_cl) %>% 
  group_by(zona, pre_post_quema) %>% 
  summarise_at(vars(humedad:p_h_k_cl), ~mean(., na.rm=TRUE)) %>% 
  pivot_longer(humedad:p_h_k_cl, names_to = "var", values_to = "mean")

errores <- soil %>% dplyr::select(
  pre_post_quema, zona, humedad:p_h_k_cl) %>% 
  group_by(zona, pre_post_quema) %>% 
  summarise_at(vars(humedad:p_h_k_cl), ~plotrix::std.error(., na.rm=TRUE)) %>% 
  pivot_longer(humedad:p_h_k_cl, names_to = "var", values_to = "se")
